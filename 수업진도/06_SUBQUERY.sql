...
SELECT
     ...
     , E.SALARY 급여
  FROM EMPLOYEE E
 WHERE SALARY > (SELECT FLOOR(AVG(E2.SALARY))
                   FROM EMPLOYEE E2
                 );
...
-- IN / NOT IN 연산자 : 여러 개 결과값 중 한 개라도 일치하면 TRUE, 아니면 FALSE
-- > ANY , < ANY : 여러 개의 결과값 중에서 한 개라도 크거나 작은 경우
--                 가장 작은 값보다 큰가? / 가장 큰 값보다 작은가?
-- > ALL , < ALL : 모든 값보다 크거나 작은 경우
--                 가장 큰 값보다 큰가? / 가장 작은 값보다 작은가?
-- EXISTS / NOT EXISTS : 값이 존재하나? / 존재하지 않는가?
...
SELECT
     ...
     , E.HIRE_DATE
  FROM EMPLOYEE E
 WHERE (E.JOB_CODE, E.DEPT_CODE) IN (SELECT E2.JOB_CODE
                                          , E2.DEPT_CODE
                                       FROM EMPLOYEE E2
...
SELECT
  ...
  JOIN EMPLOYEE E ON(V.JOBVNG = E.SALARY AND E.JOB_CODE = V.JOB_CODE)
  JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
 ORDER BY J.JOB_NAME;
...
SELECT
 ...
 WHERE ROWNUM < 5;
-- 상위구별가능
-- WHERE ROWNUM BETWEEN 6 AND 10; 요런건 안됨 가상의 1부터 시작하기에... 
-- 저 자체를 또 서브쿼리로 만들어서 사이 추출 가능
...
SELECT
 ...
 -- RANK() : 동일한 순위 이후의 등수를 동일한 인원 수만큼 건너뛰고 순위를 계산하는 방식
...
SELECT
 ...
 WHERE V.순위 BETWEEN 6 AND 10;   
-- 중복은 활용하지 못함
-- STOPKEY 활용 불가능, ROWNUM 활용보단 성능 떨어짐

...
SELECT
       D.DEPARTMENT_NAME || '의 정원은 ' || D.CAPACITY || '명 입니다.' AS "학과별 정원"
  FROM TB_DEPARTMENT D;
...
SELECT
       S.STUDENT_NAME
  FROM TB_STUDENT S
 WHERE S.STUDENT_NO = 'A513079'
    OR S.STUDENT_NO = 'A513090'
    OR S.STUDENT_NO = 'A513091'
    OR S.STUDENT_NO = 'A513110'
    OR S.STUDENT_NO = 'A513119'
 ORDER BY 1 DESC;
...
SELECT
       C.CLASS_NO
  FROM TB_CLASS C
 WHERE C.PREATTENDING_CLASS_NO IS NOT NULL;
...
SELECT
     ...
     , P.PROFESSOR_SSN
  FROM TB_PROFESSOR P
 WHERE LENGTH(P.PROFESSOR_NAME) <> 3;
...
SELECT
     ...
     , EXTRACT(YEAR FROM SYSDATE) - (1900 + SUBSTR(P.PROFESSOR_SSN, 1, 2)) 나이
  FROM TB_PROFESSOR P
 WHERE SUBSTR(P.PROFESSOR_SSN, 8, 1) = 1
 ORDER BY 2;
...
SELECT
     ...
     , S.STUDENT_NAME
  FROM TB_STUDENT S
 WHERE (EXTRACT(YEAR FROM S.ENTRANCE_DATE) 
     - TO_CHAR(TO_DATE(SUBSTR(S.STUDENT_SSN, 1, 6), 'RRMMDD'), 'RRRR')) > 19;
-- 6. 2020년 크리스마스는 무슨 요일인가?
SELECT TO_CHAR(TO_DATE(20201225), 'DAY') AS "2020년크리스마스" FROM DUAL; 
...
SELECT
       TO_CHAR(TO_DATE('99/10/11','YY/MM/DD'), 'YYYY"년" MM"월" DD"일"') AS "99/10/11 YY/MM/DD"
     , TO_CHAR(TO_DATE('49/10/11','YY/MM/DD'), 'YYYY"년" MM"월" DD"일"') AS "49/10/11 YY/MM/DD"
     , TO_CHAR(TO_DATE('99/10/11','RR/MM/DD'), 'RRRR"년" MM"월" DD"일"') AS "99/10/11 RR/MM/DD"
     , TO_CHAR(TO_DATE('49/10/11','RR/MM/DD'), 'RRRR"년" MM"월" DD"일"') AS "49/10/11 RR/MM/DD"
  FROM DUAL;
...
SELECT
     ...
     , COUNT(S.DEPARTMENT_NO) AS "학생수(명)"
  FROM TB_STUDENT S
...
SELECT
     ...
     , COUNT(DECODE(S.ABSENCE_YN, 'Y', 1, NULL)) 휴학생수
  FROM TB_STUDENT S
 GROUP BY S.DEPARTMENT_NO
 ORDER BY 1;
...
SELECT
     ...
     , COUNT(*)
  FROM TB_STUDENT S
 WHERE S.STUDENT_NAME = S.STUDENT_NAME
 GROUP BY S.STUDENT_NAME
HAVING COUNT(*) > 1
 ORDER BY 1;

-- (못품)15. 학번이 A112113 인 김고운 학생의 
...
SELECT
     ...
     , G.POINT "평점"
  FROM TB_GRADE G
 WHERE SUBSTR(G.TERM_NO, 1, 4) BETWEEN 2001 AND 2004
...
SELECT
 ...
 ORDER BY SUBSTR(S.STUDENT_SSN, 1, 6) DESC;
...
SELECT
 ...
 WHERE (STUDENT_ADDRESS LIKE '%강원도%'
   ...
   AND SUBSTR(ENTRANCE_DATE, 1, 1) > 0
 ORDER BY 1;
...
SELECT
 ...
 WHERE S.DEPARTMENT_NO = (SELECT D.DEPARTMENT_NO FROM TB_DEPARTMENT D WHERE D.DEPARTMENT_NAME = '음악학과')
...
SELECT
 ...
 WHERE SUBSTR(TERM_NO, 1, 4) = '2007'
   AND G.CLASS_NO = (SELECT C.CLASS_NO FROM TB_CLASS C WHERE C.CLASS_NAME = '인간관계론');
...
SELECT
  ...
  WHERE C.CLASS_TYPE LIKE '%전공%'
    AND C.DEPARTMENT_NO = (SELECT D.DEPARTMENT_NO
                             FROM TB_DEPARTMENT D
...
-- (못품)18. 국어국문학과에서 총 평점이 가장 높은 학생의 이름과 학번을 표시하는 SQL문을 작성하시오.
